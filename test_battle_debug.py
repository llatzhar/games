...
from game_state import GameState, Player, Enemy
...
def test_battle_initiative():
    ...
    for i, char in enumerate(all_characters):
        ...
        print(f"  {i+1}. {char_type} - Initiative: {char.initiative}, Life: {char.life}, Attack: {char.attack}")
    ...
    for attacker in initiative_order:
        ...
        if attacker in players:
            ...
            if alive_enemies:
                ...
                print(f"  → Enemy({target.ai_type})に{damage}ダメージ (残りHP: {target.life})")
            else:
                print("  → 攻撃対象なし")
        else:
            # 敵の攻撃
