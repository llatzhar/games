...
class BattleIndividualAttackState(BattleGameState):
    ...
    def setup_attack_animation(self):
        ...
        if self.context.current_attacker in self.context.battle_players:
            # プレイヤーの位置
            player_index = self.context.battle_players.index(
            ...
            # 敵の位置
            enemy_index = self.context.battle_enemies.index(
                self.context.current_attacker
            )
            self.attacker_start_pos = (
                pyxel.width - 80,
                pyxel.height // 2 + 20 + enemy_index * 40,
            )
    ...
    def draw_battle_characters(self):
        ...
        if self.context.current_attacker and self.attacker_start_pos:
            # 攻撃者のインデックスを特定
            if self.context.current_attacker in self.context.battle_players:
                attacker_index = self.context.battle_players.index(
                    self.context.current_attacker
                )
                ...
                attacker_index = self.context.battle_enemies.index(
                    self.context.current_attacker
                )
            ...
            if self.context.current_attacker in self.context.battle_players:
                ...
                pyxel.text(
                    int(anim_x) - len(name_text) * 2, int(anim_y) - 25, name_text, 11
                )

                displayed_life = self.context.get_displayed_life(
                    self.context.current_attacker, initial_life, "player"
                )
                life_text = (
                    f"HP: {displayed_life}/{self.context.current_attacker.max_life}"
                )
                pyxel.text(
                    int(anim_x) - len(life_text) * 2, int(anim_y) - 15, life_text, 7
                )
            else:
                attacker_index = self.context.battle_enemies.index(
                    self.context.current_attacker
                ...
                displayed_life = self.context.get_displayed_life(
                    self.context.current_attacker, initial_life, "enemy"
                )
                life_text = (
                    f"HP: {displayed_life}/{self.context.current_attacker.max_life}"
                )
